...
int main(int argc, char *argv[]){
  ...
  tree->SetAutoFlush(10000);    // 1万イベントごとにフラッシュ
  /*
  ...
  */
  ...
  //add branch maeato
  //sprintf(bnam,"ADC_sum[%d][%d]/I",N_FADC_BOARD,N_CH);
  //tree->Branch("ADC_sum",ADC_sum,bnam);
  ...
  for(int i=0;i<32;i++){
    ...
    //MADC_hist[i]=new TH1D(hnam,hnam,200,100,4100);   //MADC Los Reso
  }
  TH2D* pid_1st[16];
  for(int i=0; i<16; i++){
...
switch(seg_id.Module){
  ...
  for(int ch=0;ch<nch[board];++ch){
    ...
    for(int i=0;i<(int)fadc_raw_data[board][ch].size();++i){
      ...
      //adc_sum[board][ch] += data_bsub[i];
      if(ADC[board][ch]<data_bsub[i]){
        ADC[board][ch]=data_bsub[i];
        PeakClk[board][ch]=i;
      }
    }
    //    cout<< adc_sum[board][ch]<<endl;
    //ADC_sum[board][ch] = adc_sum[board][ch];
